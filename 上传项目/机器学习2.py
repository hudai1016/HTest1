...
def main():
    ...
    hat_test_label2=log.predict(test_data)# 利用训练数据训练模型
    hat_test_label = clf.predict(test_data)  # 对x_test数据进行预测
    hat_test_label1=tree.predict(test_data)
    # precision:精准率，recall：召回率
    print(classification_report(test_label, hat_test_label))
    print(classification_report(test_label,hat_test_label1))
    print(classification_report(test_label,hat_test_label2))
